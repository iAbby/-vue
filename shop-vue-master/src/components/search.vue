...
export default {
  ...
  methods: {
     ...
     keyboradWatch(){
         this.value == '' || this.value == null ?  this.showData = true : this.showData = false;
         this.value == '' || this.value == null ?  this.sshowData = false : this.sshowData = true;
     },
     ...mapActions([
      'searchA'
    ]),
  ...
  beforeCreate() {
     this.axios.get('./static/data.json').then((res)=>{
          if( res.status == 200 ) {
                ...
                this.broadcastData = data.broadcast;
  ...
  created() {
     console.log(this.showData,this.search_show);
  }
}
</script>
