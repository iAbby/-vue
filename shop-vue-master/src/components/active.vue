      ...
      this.disabled = true;
       setTimeout(() => {
         for (let i = 0; i < 10; i++) {
            ...
            this.activeTitleData.push(this.activeTitleData[i]);
            this.daysData.push(this.daysData[i]); 
        }
        this.disabled = false;
       }, 200);
    },
  ...
  beforeCreate() {
     this.axios.get('./static/data.json').then((res)=>{
          if( res.status == 200 ) {
                ...
                this.daysData = data.active.days
                switch(this.tabs){
                    case '今日推荐' :
                        this.activeTitleData = data.active.tuijian_title;
                        this.imageList = data.imageList;
                      break;
                    case '时尚' :
                        this.activeTitleData = data.active.shishang_title;
                        this.imageList = data.shishangImglist;
                      break;
                    case '美妆' :
                        this.activeTitleData = data.active.meizhuang_title;
                        this.imageList = data.meizhuangImglist;
                      break;
                    case '家电' :
                        this.activeTitleData = data.active.jiadian_title;
                        this.imageList = data.jiadianImglist;
                      break;
                    case '家居' :
                        this.activeTitleData = data.active.jiaju_title;
                        this.imageList = data.jiajuImglist;
                      break;
                    case '国际' :
                        this.activeTitleData = data.active.guoji_title;
                        this.imageList = data.guojiImglist;
                      break;
                    case '生活' :
                        this.activeTitleData = data.active.shenghuo_title;
              ...
              //  console.log(res.data.home.active,this.daysData,this.activeTitleData)
           } else {
             this.imageList = this.src;
             this.broadcastData = "暂无消息~~QAQ~"
           }
            },(err)=>{
            this.imageList = this.src;
            this.broadcastData = "暂无消息~~QAQ~"
